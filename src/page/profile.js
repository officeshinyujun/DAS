...
function Profile() {
    ...
    const loadData = async (decodedData) => {
        ...
        try {
            const userResponse = await fetch(`http://127.0.0.1:8000/api/users,${decodedData.name}`);
            if (!userResponse.ok) {
            ...
            const recordResponse = await fetch(`http://127.0.0.1:8000/api/users,${decodedData.name},summaries`);
